...
describe('Products (e2e)', () => {
	...
	describe('POST /products', () => {
		...
		it('should return 409 when productToken already exists', async () => {
			...
			await request(app.getHttpServer()).post('/products').send(validProduct).expect(201);
		...
		it('should return 400 for missing required fields', async () => {
			...
			await request(app.getHttpServer()).post('/products').send(incompleteProduct).expect(400);
		});
		...
		it('should return 400 for extra fields (forbidNonWhitelisted)', async () => {
			...
			await request(app.getHttpServer()).post('/products').send(productWithExtraFields).expect(400);
		});
	});
	...
	describe('GET /products', () => {
		beforeEach(async () => {
			...
			await request(app.getHttpServer()).post('/products').send(validProduct2).expect(201);
		});
		...
		it('should return 400 for invalid pagination parameters', async () => {
			await request(app.getHttpServer()).get('/products?page=0&limit=101').expect(400);
		});
	...
	describe('DELETE /products/:id', () => {
		...
		it('should return 204 for non-existent product (idempotent)', async () => {
			await request(app.getHttpServer()).delete('/products/99999').expect(204);
		});

		it('should return 400 for invalid id format', async () => {
			await request(app.getHttpServer()).delete('/products/invalid-id').expect(400);
		});
	});
	...
	describe('Integration scenarios', () => {
		it('should handle complete CRUD workflow', async () => {
			...
			// Verify deletion
			const finalListResponse = await request(app.getHttpServer()).get('/products').expect(200);
